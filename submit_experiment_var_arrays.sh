...
NUM_LINES=$(($(wc -l < "$CSV_FILE") - 1))   # Datenzeilen ohne Header
...
for ((offset=0; offset<NUM_LINES; offset+=MAX_PER_ARRAY)); do
    count=$(( NUM_LINES - offset ))
    (( count > MAX_PER_ARRAY )) && count=$MAX_PER_ARRAY
    end=$(( count - 1 ))  # lokale IDs 0..end (<= 69999)

    echo " -> sbatch --array=0-$end (OFFSET=$offset)"
    sbatch --array="0-$end" \
           --export=ALL,RUN_ID="$RUN_ID",OFFSET="$offset",NUM_LINES="$NUM_LINES" \
           run_single_job_var_arrays.sh
done
